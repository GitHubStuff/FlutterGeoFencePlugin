...
class GeofencePlugin: NSObject {
    ...
    private static var instance: GeofencePlugin? = nil
...
extension GeofencePlugin {  //User defaults
    ...
    // Handle number
    private func getCallbackDispatchHandle() -> Int {
...
extension GeofencePlugin: FlutterPlugin {
    ...
    private func startFenceService(handle: Int64) {
    ...
    private func sendLocationEvent(region: CLRegion, eventType: EventType) {
        ...
        if let center = (region as? CLCircularRegion)?.center {
            ...
            callbackChannel.invokeMethod("", arguments: [handle, region.identifier, center.latitude, center.longitude, eventType])
        }
    }
